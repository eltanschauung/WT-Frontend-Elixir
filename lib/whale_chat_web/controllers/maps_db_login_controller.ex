defmodule WhaleChatWeb.MapsDbLoginController do
  ...
  def show(conn, params) do
    ...
    return_to = sanitize_return(params["return"] || "/mapsdb")
  ...
  defp login_or_callback(conn, %{"steamid" => steamid}, return_to) when is_binary(steamid) do
    # Optional local testing bridge; real Steam OpenID is the default path now.
  ...
  defp login_or_callback(conn, %{"openid.mode" => _mode} = params, return_to) do
  ...
  defp absolute_login_return_url(conn, return_to) do
    request_realm(conn) <> "/mapsdb/login.php?return=" <> URI.encode(return_to)
  end
  ...
  defp append_query_flag(path, key, value) do
    ...
    new_query = URI.encode_query(Map.put(existing, key, value))
  ...
  defp render_error_page(conn, return_to, error_msg) do
    ...
    error_html = "<p class=\"err\">" <> esc.(error_msg) <> "</p>"
    steamid_text = esc.(steamid || "none")
    return_text = esc.(return_to)
    ...
    <html lang="en"><head>
      ...
      #{error_html}
      <p class="muted">Current session steamid: <code>#{steamid_text}</code> (admin: <code>#{if admin, do: "yes", else: "no"}</code>)</p>
      <p><a href="/mapsdb/login.php?return=#{URI.encode(return_to)}">Try Steam login again</a></p>
      <p><a href="#{return_text}">Return to mapsdb</a></p>
    </div></body></html>
    """
